...
- (void)setStatus:(WBStatus *)status
{
    ...
    CGSize timeLabelSize = [status.createdTime sizeWithFont:WBStatusTimeFont];
    _timeLabelF = (CGRect){{timeLabelX, timeLabelY}, timeLabelSize};
    
    // 6.来源
    CGFloat sourceLabelX = CGRectGetMaxX(_timeLabelF) + WBStatusCellBorder;
    CGFloat sourceLabelY = timeLabelY;
    CGSize sourceLabelSize = [status.source sizeWithFont:WBStatusTimeFont];
...
#warning 根据图片个数计算整个相册的尺寸
    ...
    if (status.retweeted_status) {
        ...
        CGSize retweetNameLabelSize = [name sizeWithFont:WBStatusNameFont];
        ...
        CGSize retweetContentLabelSize = [status.retweeted_status.text sizeWithFont:[UIFont systemFontOfSize:13] constrainedToSize:CGSizeMake(retweetContentLabelMaxW, MAXFLOAT)];
